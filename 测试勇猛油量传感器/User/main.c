...
volatile unsigned int TempSendOil;
unsigned char CampareOil = 0;
unsigned char Cazhi = 0;
unsigned char m = 0;
...
unsigned char CounterSendtimer = 0;
...
//
#if 0
		if(CampareOil > youliang)
		{
			TempSendOil = CampareOil - m;
			//
			
		}
		else if(CampareOil < youliang)
		{
			 	TempSendOil = CampareOil + m;
		}
		else if(CampareOil == youliang)
		{
			  	TempSendOil = CampareOil;
		}
...
//
	...
	WDT_Open(WDT_TIMEOUT_2POW16, WDT_RESET_DELAY_3CLK, TRUE, FALSE);
	...
	while(g_u8IsWDTTimeoutINT == 0)
	{
		...
		for(m=0; m < 6; m++)
		{
			if(CounterSendtimer == 39)
			{		
				//UART_SendData();
				//printf("\r\n 66666666666666668888888888888888 \r\n");
